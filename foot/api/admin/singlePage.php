<?php 
    ...
    if($action == 'singlePageDeleteByIds' && $ids ) {
        // $ids = '{"ids":["5","4","3","2","1"]}' json字符串
        $ids = json_decode($ids,true);
        // 我们想要的sql语句效果 delete from table_name where id in (1,2,3,4)
	
        // 查询图片循环删除
        $oldpic = "select pic from {$pre}single_page where single_id in (${ids['ids']})"; //array
    ...
    if($action == 'singlePageDelete' && $id ) {
        $oldpic= "select pic from f_single_page where single_id = {$id}";
    ...
    if($action == 'singlePage'){
        $sql = "select * from ${pre}single_page order by single_id desc limit ${offset}, ${limit}";
        $data = getAll($sql);
        echo json_encode($data);die;
    }

    if($action == 'singlePageCount') {
    ...
    if($action == 'singlePageAdd'){
        if($_POST){
            ...
            if(!$id){
                ...
                $result = update("singlePage",$data,"where single_id=${id}");
                if($result){
                    echo_json(1,'更新成功');
                }else{
                    echo_json(0,'更新失败');
