<?php 
    ...
    if($action == 'newsDeleteByIds' && $ids ) {
        ...
        // 查询图片循环删除
        $oldpic = "select pic from {$pre}news where news_id in (${ids['ids']})"; //array
    ...
    if($action == 'newsDelete' && $id ) {
        // "select pic from f_news where news_id = 28"
        // $oldpic= str_replace(HTTP, ROOT, $oldpic);
        // unlink($oldpic);
        $Pic= "select pic from f_news where news_id = {$id}";
        $Pic = getOne($Pic);
    ...
    if($action == 'newsAdd'){
        if($_POST){
            ...
            $create_time = isset($_POST['create_time']) && !empty($_POST['create_time']) ? ($_POST['create_time']) : '';
