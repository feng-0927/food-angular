<?php 
    ...
    if($action == 'friendLinkDeleteByIds' && $ids ) {
        ...
        // 查询图片循环删除
        $oldpic = "select pic from {$pre}friend_link where link_id in (${ids['ids']})"; //array
    ...
    if($action == 'friendLinkDelete' && $id ) {
        // "select pic from f_friendLink where link_id = 28"
        // $oldpic= str_replace(HTTP, ROOT, $oldpic);
        // unlink($oldpic);
        $oldpic= "select pic from {$pre}friend_link where link_id = {$id}";
        // var_dump($oldpic);die;
